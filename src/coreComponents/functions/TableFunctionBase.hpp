...
/**
 * @file TableFunctionBase.hpp
 */

#ifndef GEOS_FUNCTIONS_TABLEFUNCTIONBASE_HPP_
#define GEOS_FUNCTIONS_TABLEFUNCTIONBASE_HPP_
...
private:
    ...
    friend class TableFunctionBase; // Allow only parent class to construct the wrapper
    ...
    TableFunctionBase::InterpolationType m_interpolationMethod = InterpolationType::Linear;
   ...
   */
  inline real64 eval( real64 const * const input ) const
  {
  ...
  /**
   * @brief @return Coordinate lower bound for dimension
   */
  real64  lowerBound ( localIndex const dim ) const;

  /**
   * @brief @return Coordinate upper bound for dimension
   */
  real64  upperBound( localIndex const dim ) const;
  ...
  /**
   * @brief Set the interpolation method, name should be setInteroplationMethod, for backwards compat.
   * @param method The interpolation method
   */
  void setInterpolationType( InterpolationType const method );

  /**
   * @brief Check validity of enteries
   * @return Flag if table is valid, index of invalid entry, -1 if ok
   */
  std::pair< bool, localIndex > validateMonotonicIncreasing();

  /**
   * @brief Check if coordinates equals values
   * @return true/false
   */
  bool validateNumberOfEntries();

  /**
   * @brief Check if dimension is within coordinates
   * @return true/false
   */
  bool validateDimension( localIndex const dim ) const;

  /**
   * @brief Check if value is with range of coordinates for
   * @return true/false
   */
  bool validateCoordinate( real64 const coord, localIndex const dim ) const;
   ...
   */
  void setCoordinates( array1d< real64_array > const & coordinates,
   ...
   */
  void setValues( real64_array values, units::Unit unit = units::Unknown );
...
GEOS_FORCE_INLINE
real64
TableFunctionBase::KernelWrapper::compute( IN_ARRAY const & input ) const
{
  if( m_interpolationMethod == TableFunctionBase::InterpolationType::Linear )
  {
...
GEOS_FORCE_INLINE
real64
TableFunctionBase::KernelWrapper::interpolateLinear( IN_ARRAY const & input ) const
{
...
GEOS_FORCE_INLINE
real64
TableFunctionBase::KernelWrapper::interpolateRound( IN_ARRAY const & input ) const
{
  integer const numDimensions = LvArray::integerConversion< integer >( m_coordinates.size() );
  ...
  for( integer dim = 0; dim < numDimensions; ++dim )
  {
    ...
    else
    {
      ...
      if( m_interpolationMethod == TableFunctionBase::InterpolationType::Nearest )
      {
      ...
      else if( m_interpolationMethod == TableFunctionBase::InterpolationType::Lower )
      {
        if( subIndex > 0 )
        {
          --subIndex;
...
GEOS_FORCE_INLINE
real64
TableFunctionBase::KernelWrapper::compute( IN_ARRAY const & input, OUT_ARRAY && derivatives ) const
{
...
GEOS_FORCE_INLINE
real64
TableFunctionBase::KernelWrapper::interpolateLinear( IN_ARRAY const & input, OUT_ARRAY && derivatives ) const
{
  integer const numDimensions = LvArray::integerConversion< integer >( m_coordinates.size() );
...
GEOS_FORCE_INLINE
real64
TableFunctionBase::KernelWrapper::interpolateRound( IN_ARRAY const & input, OUT_ARRAY && derivatives ) const
{
...
ENUM_STRINGS( TableFunctionBase::InterpolationType,
              "linear",
              "nearest",
              "upper",
              "lower" );
...
#endif /* GEOS_FUNCTIONS_TABLEFUNCTIONBASE_HPP_ */
